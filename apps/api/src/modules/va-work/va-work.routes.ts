...
import { acceptVaWork, cancelVaWork, listVaWork, sendBackVaWork, submitVaWork } from "./va-work.service.js";

export async function registerVaWorkRoutes(app: FastifyInstance) {
  app.get("/v1/va-work-items", async () => listVaWork());
  app.post<{ Params: { itemId: string } }>("/v1/va-work-items/:itemId/accept", async (request) => acceptVaWork(request.params.itemId, parseBody(vaWorkDecisionRequestSchema, request.body ?? {})));
