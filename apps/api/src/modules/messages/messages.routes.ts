...
export async function registerMessagesRoutes(app: FastifyInstance) {
  ...
  app.patch<{ Params: { dealId: string; messageId: string } }>("/v1/deals/:dealId/messages/:messageId", async (request) => updateMessage(request.params.dealId, request.params.messageId, parseBody(updateMessageRequestSchema, request.body), getRequiredSession(request)));
}
