...
import { inviteUser, listUsers, resendInvitation, updateUser } from "./users.service.js";
...
export async function registerUsersRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { userId: string } }>("/v1/users/:userId/resend-invitation", async (request) => resendInvitation(request.params.userId, getRequiredSession(request)));
}
