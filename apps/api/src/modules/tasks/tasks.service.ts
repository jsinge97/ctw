...
export async function decideTask(taskId: string, session: CurrentSession, decision: "approve" | "reject" | "defer" | "route", input: TaskDecisionRequest = {}): Promise<TaskDto> {
  ...
  if (decision === "approve") {
    ...
    if (task.route === "system") {
      ...
      await auditService.recordApprovalEvent({ organizationId: orgId, dealId: task.dealId, taskId: task.id, messageId: message.id, approvalType: "outbound_send", decision: "approved", decisionByMembershipId: session.membership.id, ...(input.reason ? { decisionReason: input.reason } : {}) });
      if (messageStatus === "sent") await auditService.recordTaskOutcome({ organizationId: orgId, dealId: task.dealId, taskId: task.id, outcome: "completed", createdByMembershipId: session.membership.id });
      ...
      await auditService.recordApprovalEvent({ organizationId: orgId, dealId: task.dealId, taskId: task.id, approvalType: "next_action", decision: "approved", decisionByMembershipId: session.membership.id, ...(input.reason ? { decisionReason: input.reason } : {}) });
      await auditService.recordTaskOutcome({ organizationId: orgId, dealId: task.dealId, taskId: task.id, outcome: "accepted", createdByMembershipId: session.membership.id });
    }
  }
  if (decision === "reject") {
    task.status = "rejected";
    await auditService.recordApprovalEvent({ organizationId: orgId, dealId: task.dealId, taskId: task.id, approvalType: "next_action", decision: "rejected", decisionByMembershipId: session.membership.id, ...(input.reason ? { decisionReason: input.reason } : {}) });
    await auditService.recordTaskOutcome({ organizationId: orgId, dealId: task.dealId, taskId: task.id, outcome: "rejected", createdByMembershipId: session.membership.id });
  }
  if (decision === "defer") {
    task.status = "deferred";
    await auditService.recordTaskOutcome({ organizationId: orgId, dealId: task.dealId, taskId: task.id, outcome: "deferred", createdByMembershipId: session.membership.id });
  }
  if (decision === "route" && input.route) {
    task.route = input.route;
    await auditService.recordTaskOutcome({ organizationId: orgId, dealId: task.dealId, taskId: task.id, outcome: "rerouted", createdByMembershipId: session.membership.id });
  }
