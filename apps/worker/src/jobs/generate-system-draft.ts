...
export async function generateSystemDraftRecord(input: { organizationId: string; taskId: string; dealId: string }) {
  if (process.env.CTW_DB_MODE !== "prisma") return generateSystemDraft({ dealTitle: input.dealId, taskTitle: input.taskId });
  const prisma = getPrismaClient();
  const task = await prisma.task.findFirst({ where: { id: input.taskId, dealId: input.dealId, organizationId: input.organizationId }, include: { deal: true } });
